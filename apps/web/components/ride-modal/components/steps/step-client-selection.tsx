...
export function StepClientSelection({
  ...
  const showInitialClientLoading =
    isLoadingClientDirectory && !hasVisibleClients;
  ...
  const gridItems: ClientGridItem[] = [
    ...[...clients.filter(Boolean)].sort((a, b) =>
    ...
    >
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <span className="text-[10px] text-text-secondary font-bold">
          {clientDirectoryMeta?.returned ?? clients.length} disponíveis
        </span>
      </div>
      ...
      !isClientDirectoryError &&
      isClientDirectoryReady &&
      clientDirectoryMeta?.hasMore &&
      !clientSearch.trim() ? (
        <p className="text-xs font-medium text-text-secondary">
          Mostrando {clientDirectoryMeta.returned} clientes. Digite para buscar
          mais.
        </p>
      ) : null}
              ...
              if (isCreateButton(item)) {
                return (
                  ...
                  >
                    <Plus
                      size={20}
                      className="text-icon-info group-hover:scale-110 transition-transform"
                    />
                    <span className="text-[10px] text-icon-info mt-1 font-bold uppercase tracking-tighter">
                      Novo
                    </span>
                  </button>
                );
                ...
                >
                  <div className="absolute top-2 left-2">
                    <Users
                      ...
                      className={cn(
                        isSelected
                          ? "text-button-primary-foreground/40 fill-button-primary-foreground/20"
                          : "text-icon-info fill-icon-info/20",
                      )}
                    />
                  </div>
                  ...
                  {client.isPinned && (
                    ...
                    >
                      <Star
                        ...
                        className={
                          isSelected
                            ? "fill-button-primary-foreground/50"
                            : "fill-icon-warning"
                        }
                      />
                    </div>
                  ...
                  <span
                    className={cn(
                      ...
                      isSelected
                        ? "text-button-primary-foreground"
                        : "text-icon-info group-hover:text-icon-info-hover",
                    )}
                  >
        ...
        {isCreatingClient && (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-display font-extrabold text-text-primary tracking-tight">
                Novo Cliente
              </h3>
              <button
                type="button"
