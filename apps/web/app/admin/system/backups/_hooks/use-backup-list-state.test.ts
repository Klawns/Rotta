...
const viewModels = [
  ...
  },
  {
    ...
    createdAtRelativeLabel: 'ha 1h',
    sourceLabel: 'Automatico',
    sizeLabel: '--',
...
test('filters backups by status and origin together', () => {
  ...
  assert.deepEqual(results.map((item) => item.id), ['backup-2']);
});
...
test('clamps and paginates pages safely', () => {
  ...
  assert.deepEqual(paginated.map((item) => item.id), ['backup-9']);
});
