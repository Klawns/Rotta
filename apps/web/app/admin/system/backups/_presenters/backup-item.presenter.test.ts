...
test('maps successful backups into concise list item view models', () => {
  ...
  assert.equal(viewModel.sourceLabel, 'Automatico');
  assert.equal(viewModel.sizeLabel, '2,00 MB');
  assert.equal(viewModel.status.label, 'Concluido');
...
test('marks failed backups as non-downloadable and keeps fallback labels', () => {
  ...
  assert.equal(viewModel.createdAtLabel, 'Data indisponivel');
  assert.equal(viewModel.checksumLabel, 'Checksum indisponivel');
