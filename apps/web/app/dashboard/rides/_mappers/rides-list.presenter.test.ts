...
test('uses archived-specific copy when the archived scope has no rides', () => {
  const presenter = buildRidesListPresenter({
    rides: [],
    totalCount: 0,
    isLoading: false,
  ...
  assert.equal(presenter.emptyTitle, 'Nenhuma corrida arquivada');
  assert.equal(presenter.resultsLabel, '0 corridas arquivadas');
});
