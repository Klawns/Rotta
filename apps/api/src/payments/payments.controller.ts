...
export class PaymentsController {
    ...
    @Post('checkout')
    async createCheckout(@Body() body: { plan: PaymentPlan, couponCode?: string }, @Req() req: any) {
        const userId = req.user.id;
        return this.paymentsService.createCheckoutSession(userId, body.plan, body.couponCode);
    }
    ...
    async handleWebhook(
        ...
        console.log(`[Webhook] Recebido! Signature: ${!!signature}, rawBody: ${!!request.rawBody}`);
        if (!signature || !request.rawBody) {
            console.error('[Webhook] Falha na validação inicial: Assinatura ou corpo ausente.');
