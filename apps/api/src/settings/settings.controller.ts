...
export class SettingsController {
    ...
    @Post('ride-presets')
    async createRidePreset(@Request() req: any, @Body() body: { label: string, value: number, location: string }) {
        return this.settingsService.createRidePreset(req.user.id, body);
    }

    @Patch('ride-presets/:id')
    async updateRidePreset(@Request() req: any, @Param('id') id: string, @Body() body: Partial<{ label: string, value: number, location: string }>) {
